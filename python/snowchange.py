...
def snowchange(
    ...
    snowflake_role,
    snowflake_warehouse,
    change_history_table_override,
    vars,
    autocommit,
    verbose,
    clone,
):
    # Checking validity of root folder provided
    ...
    print("Using Snowflake username %s" % snowflake_user)
    ...
    create_change_history_table_if_missing(
        change_history_table, autocommit, verbose, snowflake_password
    )
    print(
        "Using change history table %s.%s.%s"
        % (
    ...
    change_history = fetch_change_history(
        change_history_table, autocommit, verbose, snowflake_password
    )
    if change_history:
    ...
    all_script_names_sorted = sorted_alphanumeric(
    ...
    # Drop SnowCloned databases
    drop_snowcloned_databases("", autocommit, verbose, clone, snowflake_password)
    ...
    for (directory_path, directory_names, file_names) in os.walk(root_directory):
        for file_name in file_names:
            ...
            script_name_parts = re.search(
                r"^([V])(.+)__(.+)\.(?:sql|SQL)$", file_name.strip()
            )
            repeatable_script_name_parts = re.search(
                r"^([R])__(.+)\.(?:sql|SQL)$", file_name.strip()
            )
            ...
            script["script_version"] = (
                None if script_type == "R" else script_name_parts.group(2)
            )
            script["script_description"] = (
                (
                    repeatable_script_name_parts.group(2)
                    if script_type == "R"
                    else script_name_parts.group(3)
                )
                .replace("_", " ")
                .capitalize()
            )
...
def execute_queries_on_cloned_databases(
    vars, content, autocommit, verbose, clone, snowflake_password
):
    clone_vars = vars.copy()
    clone_query = ""
    if clone:
        try:
            for k, v in clone_vars.items():
                # Create queries of clones of QA databases
                clone_query = (
                    clone_query
                    + f"CREATE TRANSIENT DATABASE IF NOT EXISTS {v + '_snowclone'} CLONE {v};"
                )

                # Create clone_vars for clone databases
                clone_vars[k] = v + "_snowclone"
        ...
        if verbose:
            print(f"Cloned variables:  {clone_vars}")

        # Create transient clone databases
        execute_snowflake_query(
            "", clone_query, autocommit, verbose, snowflake_password
        )

        # Replace any variables used in the script content
        content = replace_variables_references(content, clone_vars, verbose)

        # Run cloned scripts on cloned databases
        execute_snowflake_query("", content, autocommit, verbose, snowflake_password)


def drop_snowcloned_databases(
    snowflake_database, autocommit, verbose, clone, snowflake_password
):
    drop_query = ""
    database_list = ""
    if clone:
        ...
        if verbose:
            print("Removing all SnowCloned databases created by snowchange.")

        # Getting list of all snowcloned databases (i.e. created by snowchange)
        database_list = (
            "SELECT DATABASE_NAME FROM LZ_DEV.INFORMATION_SCHEMA.DATABASES WHERE UPPER(DATABASE_NAME) LIKE '%_SNOWCLONE'"
        ...
        if snowflake_password is not None:
            ...
            raise ValueError(
                "Unable to find connection credentials for password authentication"
            )
        ...
        cur = con.cursor()
        # Fetch the results for database list
        db_list = cur.execute(database_list).fetchall()
        print(db_list)

        # Preparing the drop database quey
        for i in db_list:
            drop_query = drop_query + f"DROP DATABASE {i[0]};"

        # Be a good samartian and close connections
        con.close()

        # Dropping all snowcloned databases (created by snowchange)
        execute_snowflake_query("", drop_query, autocommit, verbose, snowflake_password)


def execute_snowflake_query(
    snowflake_database, query, autocommit, verbose, snowflake_password
):
    # Password authentication is the default
    if snowflake_password is not None:
        if verbose:
...
def get_change_history_table_details(change_history_table_override):
    ...
    if change_history_table_override is not None:
        ...
        if len(table_name_parts) == 1:
            ...
            raise ValueError(
                "Invalid change history table name: %s" % change_history_table_override
            )
...
def create_change_history_table_if_missing(
    ...
    execute_snowflake_query("", query, autocommit, verbose, snowflake_password)
...
def fetch_change_history(change_history_table, autocommit, verbose, snowflake_password):
...
def apply_change_script(
    script, vars, change_history_table, autocommit, verbose, clone, snowflake_password
):
    ...
    # Execute the contents of the script on database clone
    if len(content) > 0:
        execute_queries_on_cloned_databases(
            vars, content, autocommit, verbose, clone, snowflake_password
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "--snowflake-password",
        type=str,
        help="The password of the snowflake user",
        required=True,
    )
    parser.add_argument(
        "-r",
    ...
    parser.add_argument("-cl", "--clone", action="store_true")
    ...
    snowchange(
        ...
        args.snowflake_user,
        args.snowflake_password,
        args.snowflake_role,
        args.snowflake_warehouse,
        args.change_history_table,
        args.vars,
        args.autocommit,
        args.verbose,
        args.clone,
